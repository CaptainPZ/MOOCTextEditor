...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		while(!curr.getWord().equals(word2.toLowerCase()) && !queue.isEmpty()) {
			curr = queue.remove(0);
			List<String> oneMutuation = nw.distanceOne(curr.getWord(), true);
			for (String w : oneMutuation) {
				if(!visited.contains(w)) {
					WPTreeNode tmp = curr.addChild(w);
					visited.add(w);
					queue.add(tmp);					
				}
				if(w.equals(word2)) {
					return curr.addChild(w).buildPathToRoot();
				}
			}

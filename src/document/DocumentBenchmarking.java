...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			String contents = getStringFromFile(textfile, numToCheck);
			
			//start the time watch for basic
			long startTime = System.nanoTime();
			for(int i = 1; i<=trials;i++) {
				Document basicTester = new BasicDocument(contents);
				double basicScore = basicTester.getFleschScore();
			}
			long endTime = System.nanoTime();
			double estTime = (endTime - startTime) / 1000000000.0;
			System.out.print(estTime + "\t");
			
			//start the time watch for efficient
			startTime = System.nanoTime();
			for(int i = 1; i<=trials;i++) {
				Document efficientTester = new EfficientDocument(contents);
				double efficientScore = efficientTester.getFleschScore();
			}
